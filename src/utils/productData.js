...
const getApiBaseUrl = () => {
  ...
  if (isLocalhost) {
    ...
    console.log(
      "🌐 Auto-detected DEPLOYED environment, using:",
      DEPLOYED_API_URL
    );
    return DEPLOYED_API_URL;
  }
...
const testConnection = async () => {
  try {
    ...
    if (API_BASE_URL.includes("localhost")) {
      console.log(
        "💡 Localhost API not available. You can switch to deployed API by running: switchToDeployedAPI()"
      );
    }
    return false;
  }
...
export const validateStockForCart = async (productId, quantity) => {
  try {
    console.log(
      `🔍 Validating stock for product ${productId}, quantity: ${quantity}`
    );
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 404) {
        console.error(
          "❌ API endpoint not found. Check if backend is deployed correctly."
        );
        return {
          allAvailable: false,
          error:
            "Stock validation service temporarily unavailable. Please try again.",
    ...
    return {
      allAvailable: false,
      error:
        "Network error - unable to validate stock. Please check your connection and try again.",
      items: items.map((item) => ({
        productId: item.productId,
        available: false,
        error: "Network error",
...
export const createOrderWithStockValidation = async (orderData) => {
  try {
    console.log(
      "💳 Creating order with stock validation at:",
      `${API_BASE_URL}/orders/with-stock-validation`
    );
